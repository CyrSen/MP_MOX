...
document.addEventListener("DOMContentLoaded", function () {
    ...
    image.addEventListener("click", function (event) {
        ...
        iconElement.classList.add(
            ...
            "tooltip-icon"
        ); // Add classes for the desired icon

        // Remove existing location dots
        var existingLocationDots = document.getElementsByClassName(
            "locationIconFeedback"
        );
...
function addOverlayIcon(event, category) {
    ...
    const categoryIcons =
        categoryWrapper.getElementsByClassName("icon-element");
    for (let i = 0; i < categoryIcons.length; i++) {
        const categoryIcon = categoryIcons[i];
        const existingOverlayIcons =
            categoryIcon.querySelectorAll(".overlay-icon");
